...
export const globalFilter = <T extends object>(
  ...
  if (value && typeof value === 'object') {
    return getObjectStringContents(value)
      ?.toLowerCase()
...
export const globalSort = <T extends object>(
  a: Row<T>,
  b: Row<T>,
  columnId: string,
) => {
  ...
  if (typeof aVal === 'object' && typeof bVal === 'object') {
    ...
    const aName: string = aUnknown.name
      || aUserName
      || aUnknown.title
      || aUnknown.label
      || aUnknown.id
      || ''
    const bName: string = bUnknown.name
      || bUserName
      || bUnknown.title
      || bUnknown.label
      || bUnknown.id
  ...
  if (
    columnId?.toLowerCase().includes('date')
    && (dayjs(aVal as string).isValid() || dayjs(bVal as string).isValid())
  ) {
...
const getObjectStringContents = (value: unknown): string => {
  if (value && typeof value === 'object') {
    const stringValues: string = Object.values(value).reduce(
      (acc: string, val) => {
